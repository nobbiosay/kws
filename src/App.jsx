...
// ==========================
// Simple CSS (meets requirement)
// ==========================
const baseCss = `
  :root{--bg:#0b1220;--card:#121a2b;--muted:#8ea0bb;--accent:#39a2db;--accent2:#6ee7b7;--danger:#ef4444;--ok:#22c55e}
  *{box-sizing:border-box} html,body,#root{height:100%}
  body{margin:0;background:linear-gradient(180deg,#0b1220,#0b1220 60%,#0e1729);color:#e6eefc;font-family:Inter,system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif}
  a{color:var(--accent)}
  .container{max-width:1200px;margin:0 auto;padding:24px}
  .brand{display:flex;align-items:center;gap:12px;font-weight:800;letter-spacing:.3px}
  .brand .logo{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--accent),var(--accent2));display:grid;place-items:center;color:#00111a;font-weight:900}
  .card{background:rgba(255,255,255,.04);backdrop-filter: blur(6px);border:1px solid rgba(255,255,255,.08);border-radius:16px;box-shadow:0 10px 30px rgba(0,0,0,.25)}
  ...
  .btn{cursor:pointer;border:none;border-radius:12px;padding:10px 14px;font-weight:700;transition:.2s ease;display:inline-flex;align-items:center;gap:8px}
  .btn.primary{background:linear-gradient(135deg,var(--accent),#3f8cff);color:#00111a}
  .btn.ghost{background:transparent;border:1px solid rgba(255,255,255,.12);color:#e6eefc}
  .btn.warn{background:var(--danger);color:white}
  .btn:disabled{opacity:.5;cursor:not-allowed}
  input,select,textarea{width:100%;padding:10px 12px;border-radius:10px;background:#0f1626;border:1px solid rgba(255,255,255,.12);color:#e6eefc}
  ...
  .tab{padding:8px 12px;border-radius:999px;border:1px solid rgba(255,255,255,.12);cursor:pointer}
  .tab.active{background:rgba(57,162,219,.15);border-color:rgba(57,162,219,.4)}
...
// Catalogues (shared for Kerenceng & Cidanau)
const FLOWMETER_FIELDS = [
  ...
  ["pt_kti", "PT KTI", "m3"],
  ["kp_baru", "Kp. Baru", "m3"],
  ["warnasari", "Warnasari", "m3"],
  ["bbs", "BBS", "m3"],
  ["mandiri_ls", "Mandiri LS", "m3"],
  ["kota_panggung_rawi", "Kota & Panggung Rawi", "m3"],
  ["apbn", "APBN", "m3"],
  ["apbd", "APBD", "m3"],
  ["ps8_cipada", "PS VIII Cipada", "m3"],
...
const PUMP_GROUPS = {
  "Pompa Air Bersih (menit)": [
    43,44,45,46,47,60,61,62,63,64,70,71,72,73,74,75
  ].map(n => ["pab_"+n, `pompa ${n}`]),
...
const Nav = ({ current, setCurrent, onLogout }) => (
  <div className="header">
    <div className="brand"><span className="logo">K</span> Krakatau Water Solution</div>
    <div style={{display:'flex', gap:12, alignItems:'center'}}>
      {[
        ...
        <button key={key} className={`btn ghost ${current===key? 'active':''}`} onClick={()=>setCurrent(key)}>
          {label}
        </button>
      ))}
      <button className="btn warn" onClick={onLogout}>Logout</button>
    </div>
  </div>
);
...
const Login = ({ onLogin }) => {
  ...
  const submit = (e) => {
    e.preventDefault();
    // Demo only: accept any non-empty id/pass for Operator
  ...
  return (
    <div className="container" style={{display:'grid',placeItems:'center',minHeight:'100vh'}}>
      <div className="card panel" style={{maxWidth:560,width:'100%'}}>
        ...
        <form onSubmit={submit} className="grid" style={{gap:14}}>
          <div>
            ...
            <select value={role} onChange={(e)=>setRole(e.target.value)}>
              ...
              {/* Tempatkan role lain di masa depan */}
            </select>
          </div>
          <div>
...
const PressureHeader = ({ state, setState }) => (
  <div className="section">
    ...
    <table>
      <thead>
        <tr><th>Jam</th><th>PS IV</th><th>VI</th></tr>
      </thead>
      <tbody>
        {HOURS.map(h => (
          <tr key={h}>
...
const FilterOps = ({ state, setState }) => (
  <div className="section">
    ...
    <table>
      <thead>
        <tr><th>Unit</th><th>Durasi Runtime (menit)</th><th>Jeda Start/Stop (menit)</th></tr>
      </thead>
      <tbody>
        {FILTER_UNITS.map(u => (
          <tr key={u}>
...
const Quality = ({ state, setState }) => (
  <div className="section">
    ...
    <div className="section">
      ...
      <table>
        <thead>
          <tr><th>Shift</th><th>Acc I</th><th>Acc II</th><th>Acc III</th></tr>
        </thead>
        <tbody>
          {["I","II","III"].map((s,idx)=> (
            <tr key={s}>
...
const WtpForm = ({ plant, onSaved, initial }) => {
  ...
  return (
    <div className="grid" style={{gap:16}}>
      ...
      {tab==="flow" && (
        <SectionGrid title="Flowmeter" fields={FLOWMETER_FIELDS} state={data} setState={setData} />
      )}
      {tab==="chem" && (
        <SectionGrid title="Input Data Bahan Kimia" fields={CHEMICAL_FIELDS} state={data} setState={setData} />
      )}
      {tab==="elec" && (
        <div className="grid" style={{gap:16}}>
...
const DataTable = ({ plant, onEdit }) => {
  ...
  const cols = ["Tanggal","Flowmeter (Total m3)","Alum (kg)","Klorin (kg)","Trafo I LWBP (kWh)","Aksi"];

  const computeTotals = (data) => {
    const flow = FLOWMETER_FIELDS.reduce((sum,[key]) => sum + (Number(data[key]||0)||0), 0);
    const alum = (Number(data.alum_bak1||0) + Number(data.alum_bak2||0)) || 0;
    const klor = Number(data.pemakaian_klorin||0)||0;
    const t1 = Number(data.trafo1_lwbp||0)||0;
    return { flow, alum, klor, t1 };
  };
  ...
  return (
    <div className="section">
      <table>
        ...
        <tbody>
          {rows.length===0 && (
            <tr><td colSpan={cols.length} className="muted">Belum ada data.</td></tr>
          )}
...
const Dashboard = () => {
  ...
  const sumElec = (e)=> e? [
    ...ELECTRIC_LWBP.map(([k])=> Number(e.data[k]||0)||0),
    ...ELECTRIC_WBP.map(([k])=> Number(e.data[k]||0)||0),
  ].reduce((a,b)=>a+b,0):0;

  const totals = {
    flow: sumFlow(lastK)+sumFlow(lastC),
    chem: sumChem(lastK)+sumChem(lastC),
    elec: sumElec(lastK)+sumElec(lastC),
  };

  return (
    <div className="grid cols-3">
      {[{
        title:"Total Flowmeter (gabungan)", val: totals.flow.toLocaleString(), sub: `${lastK?lastK.date:'-'} + ${lastC?lastC.date:'-'}`
      },{
        title:"Total Bahan Kimia (Alum+Klorin)", val: totals.chem.toLocaleString()+" (kg)", sub:"Terakhir per input terbaru"
      },{
        title:"Total Listrik (LWBP+WBP)", val: totals.elec.toLocaleString()+" kWh", sub:"Estimasi agregat"
      }].map((c,i)=> (
    ...
    <>
      ...
      {user && (
        <div className="container" style={{paddingTop:18}}>
          ...
          {tab==="kerenceng-input" && (
            <div className="card panel"><WtpForm plant="kerenceng" onSaved={()=>setTab("kerenceng-data")} /></div>
          )}
          {tab==="cidanau-input" && (
            <div className="card panel"><WtpForm plant="cidanau" onSaved={()=>setTab("cidanau-data")} /></div>
          )}
...
// - Ganti localStorage dengan REST API (Node/Express, Laravel, dsb.)
